...
# ("Rowdy", 10, "M", "black", "intense", 0),
# ("Tabitha", 6, "F", "dark brown", "Nice", 0),
# ("Sargeant Brown", 19, "M", "Green", "Slimy", 1),
# ("Melissa", 13, "F", "dark brown", "goofy", 0),
# ("Grinch", 2, "M", "Black", "Grinchy", 0),
# ("Wendy", 6, "F", "Blue", "naive", 0);

# INSERT INTO bears (age, gender, color, temperament, alive) VALUES
# (20, "M", "a slightly darker black", "aggressive", 1);
  ...
  it 'counts the number of bears with goofy temperments' do
    expect(@db.execute(counts_number_of_bears_with_goofy_temperments)).to eq([[1]])
  end

  it 'selects the bear that killed Tim' do
    expect(@db.execute(selects_bear_that_killed_Tim)).to eq([[8, nil, 20, "M", "black", "aggressive", "FALSE"]])
  end
end
